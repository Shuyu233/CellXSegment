classdef CellXMembraneDetector < handle
    ...
    methods (Access=private)
        ...
        function initNLinkMatrix(this)
            ...
            this.nEdgeListVertexLindx = double(this.nEdgeListVertexLindx);
        ...
        function updateNLinkMatrix(this)
            ...
            convolutionValuesI  = this.convolutionImage(this.nEdgeListVertexLindx(:,1));
            
            if max(convolutionValuesI)>0.8
              convolutionValues  =  convolutionValuesI;
            else % scale them to -1 to 1
              convolutionValues = ( (convolutionValuesI - min(convolutionValuesI))...
                      /( max(convolutionValuesI) -min(convolutionValuesI)) - 0.5 )*2;
            end
            ...
            % updating via linear indices does not work in 32 bit windows
            % version because this.NLinkLindx contains values larger than
            % 2^31-1
            % this.NLinkSparseAdj(this.NLinkLindx) = V;
            %
            % to fix this for now, 
            % allocate a new sparse matrix instead of updating the old one
            %
            this.NLinkSparseAdj = sparse(...
                this.nEdgeListVertexLindx(:,1), ...
                this.nEdgeListVertexLindx(:,2), ...
                V,...
                this.nVertexCount,this.nVertexCount);
        end
