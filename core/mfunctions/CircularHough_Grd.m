function [seeds, varargout] = CircularHough_Grd(img, radrange, varargin)
...
prm_LM_LoBndRa = SeedSensThresh;  % minimum ratio of LM to the max of 'accum'

% make the histogram equalization prior to smoothing
if 0
    max_accum = max(accum(:));
    min_accum = min(accum(:));
...
%
% ensure that there are not too many seeds
if( size(circen,1)>MaxNumOfSeeds )
    err = MException('SeedsChk:MaxNumOfSeedsExceeded', ...
        sprintf('found %d, max=%d', size(circen,1), MaxNumOfSeeds)...
        );
    throw(err);
end
...
for k = 1 : size(circen,1),
    ...
    %figure;imagesc(accum); hold on;
    %plot(circen(k,1),circen(k,2),'ko')
    ...
    %--- start old code (produces a bug when ascnding->descending occurs one after the other)
    %SgnCv_LMmask = [ 0; 0; SgnCv_AscEdg(1:(end-2)) ] & (~SgnCv_AscEdg);
    %SgnCv_LMmask = SgnCv_LMmask & [ SgnCv_LMmask(2:end) ; 0 ];
    % ---end old code
    
    %--- start new code
    SgnCv_LMmask = zeros(length(SgnCv_AscEdg),1);
    for nri=2:length(SgnCv_AscEdg)
        if SgnCv_AscEdg(nri-1)==1 &&  SgnCv_AscEdg(nri)==0
            SgnCv_LMmask(nri)=1;
        end
    end
    % ---end new code
...
validRadii = find(cirrad>radrange(1) & cirrad<radrange(2) );

cirrad = cirrad(validRadii);
circen = circen(validRadii,:);
